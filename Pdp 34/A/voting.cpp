...
int votes[1001];
int N, M, K, maxVotes, possibleWinners;

signed main()
{   
    si(N); si(M); si(K);
    int remainingVotes = N - K;

    for(int i = 0; i < K; ++i) {
        int vote; si(vote);
        maxVotes = std::max(maxVotes, ++votes[vote]);
    }

    for(int i = 1; i <= M; ++i) {
        if(remainingVotes == 0 && votes[i] == maxVotes)
            possibleWinners++;
        else if(votes[i] + remainingVotes > maxVotes)
            possibleWinners++;
    }
    
    if(N == K) {
        if(possibleWinners > 1) printf("0\n");
        else printf("1\n");
        return 0;
    }
    printf("%d\n", possibleWinners);
